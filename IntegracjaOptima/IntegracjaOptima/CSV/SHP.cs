...
namespace IntegracjaOptima.CSV
{
    public class SHP
    {
        ...
        public SHP(Model wchodzi,int gidnumer)
        {
            _model = wchodzi;
            _gidnumer = gidnumer;   
        }

        public ModelSciezki CreateSHP()
        {
            ...
            string magazynDocelowyTestowy = Tables.Database.SqlQuery<string>($@"select TrN_Opis from cdn.TraNag where trn_trnid={_gidnumer} and TrN_TypDokumentu=306").FirstOrDefault();
            ...
            var TreElem = Tables.Database.SqlQuery<TowarIlosc>($@"select distinct isnull(TrE_TwrKod,'') as TwrKod,isnull(TrE_Ilosc,0) as Ilosc, isnull(TrE_Lp,0) as Lp,isnull(TsC_Cecha1_Wartosc,'') as NumerPartii from CDN.TraElem 
                                                                            ...
                                                                            where TrE_TrnID={_gidnumer} and TrE_TypDokumentu=306").ToList();
            ...
            {
                string trnNumerDokumentuWZout = Tables.Database.SqlQuery<string>($@"select TrN_NumerPelny from cdn.tranag where TrN_TrNID={_gidnumer} and TrN_TypDokumentu=306").FirstOrDefault();
                Logger.WriteLog($"Błąd pobierania ilości z traelem dla dokumentu: {trnNumerDokumentuWZout} , gidnumer dokumentu to: {_gidnumer}. ");
                Console.WriteLine($"Błąd pobierania ilości z traelem dla dokumentu: {trnNumerDokumentuWZout} , gidnumer dokumentu to: {_gidnumer}.");
            }
            ...
            sciezki.SciezkaWyjscia = Program.Dostep + $"BEDI_SHP_{_model.Number}.csv";
            sciezki.SciezkaWejscia = ConfigurationManager.AppSettings["OutComing"] + $"BEDI_SHP_{_model.Number}.csv";
            sciezki.Dokument = $"BEDI_SHP_{_model.Number}.csv";
